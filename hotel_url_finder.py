...
def is_exact_name_match(hotel_name: str, result_title: str) -> bool:
    ...
    # e.g., "The Residences at The Address Dubai Mall" matches "The Address Dubai Mall Residences"
    STOP_WORDS = {"the", "at", "in", "on", "of", "and", "or", "for", "to", "a", "an"}
    name_words = [w for w in name_norm.split() if w not in STOP_WORDS and len(w) > 2]
    title_words = set(title_norm.split())

    if len(name_words) >= 3:
        matched = sum(1 for w in name_words if w in title_words)
        match_ratio = matched / len(name_words)
        if match_ratio >= 0.7:
            return True
