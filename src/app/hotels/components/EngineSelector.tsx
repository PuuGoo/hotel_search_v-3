          ...
          <label
            ...
            className={`flex items-start gap-3 p-3 rounded-lg border transition-colors cursor-pointer ${
              selected.includes(engine.id)
                ...
                : "bg-panel/50 border-hairline hover:border-gray-300"
          ...
          >
            <div className="mt-0.5">
              {selected.includes(engine.id) ? (
                ...
                <FiSquare className="w-5 h-5 text-ink-soft" />
              )}
            </div>
            <input
              type="checkbox"
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {!engine.available && (
                  <span className="text-xs px-1.5 py-0.5 bg-fill text-ink rounded">
                    Không khả dụng
                  </span>
                )}
              </div>
              <p className="text-xs text-ink mt-0.5">{engine.description}</p>
            </div>
          </label>
        ))}
      </div>
